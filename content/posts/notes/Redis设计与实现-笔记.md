...
- Redis 2.8之后才提供`PSYNC`命令，支持部分同步。
- 部分同步通过复制偏移量、复制积压缓冲区（replication backlog），runId三个部分来实现
- 复制操作刚开始时，从会成为主的客户端，向主发送命令请求来执行复制步骤，后面主也会成为从的客户端用来做命令传播。
- 复制稳定后，从会每秒向主发送命令`REPLCONF ACK <offset>`，用来做心跳检测，主要有三个作用：
  - 检测主从之间的网络连接状态，在主上的INFO查看到的复制lag表示的就是上一次接收到心跳过了多少秒。
  - 辅助实现`min-slaves`配置选项，防止主在不安全的情况下执行写命令（比如从个数不够，从lag过大）。
  - 检测命令丢失，补发缺失命令。
